...
def dataloader(num_batches,
    ...
    for batch_num in range(num_batches):
        dice = torch.empty(1).uniform_(0, 1).item()
        if dice <= 0.33:
            # All batches have the same sequence length and number of reps
            seq_len = random.randint(seq_min_len, seq_max_len)
            seq_width = 4
            ...
            inp = np.zeros((seq_len + 2, batch_size, seq_width + 1))
            outp = np.zeros((max(seq_len_1, seq_len_2) + 1 , batch_size, seq_width + 1))
            inp_numbers_1 = []
            inp_numbers_2 = []
            outp_numbers = []
            ...
            for i in range(seq_len_1):
                for j in range(batch_size):
                    ...
                    inp[i, j, :seq_width] = inp_digit
            inp[seq_len_1, :, seq_width] = 1.0

            for i in range(seq_len_2):
                for j in range(batch_size):
                    inp_digit = bcd_excess_3[int(inp_numbers_2[j][i])]
                    inp[seq_len_1 + 1 + i, j, :seq_width] = inp_digit
            inp[seq_len_1 + seq_len_2 + 1, :, seq_width] = 1.0

            for i in range(max(seq_len_1, seq_len_2)):
                for j in range(batch_size):
                    outp_digit = bcd_excess_3[int(outp_numbers[j][i])]
                    outp[i, j, :seq_width] = outp_digit
            outp[max(seq_len_1, seq_len_2), :, seq_width] = 1.0



            inp = torch.from_numpy(inp)
            outp = torch.from_numpy(outp)
            ctrl = np.array([[bcd_excess_3[2]]])
            ctrl = torch.from_numpy(ctrl)
...
@attrs
class AddOneTellParityAddTaskParams(object):
    name = attrib(default="addone-tellparity-add-bcd")
...
@attrs
class AddOneTellParityAddModelTraining(object):
    params = attrib(default=Factory(AddOneTellParityAddTaskParams))
    net = attrib()
    dataloader = attrib()
    criterion = attrib()
    optimizer = attrib()
