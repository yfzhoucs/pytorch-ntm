...
@attrs
class LeftShiftTaskParams(object):
    name = attrib(default="left-shift-task")
...
@attrs
class LeftShiftTaskModelTraining(object):
    params = attrib(default=Factory(LeftShiftTaskParams))
    net = attrib()
    dataloader = attrib()
    criterion = attrib()
    optimizer = attrib()
    ...
    def input_human(self, net, number_str):
        ...
        outp = np.zeros((seq_len + 2, 1, seq_width + 1))
        net.init_sequence(1)
        # net = net.float()
