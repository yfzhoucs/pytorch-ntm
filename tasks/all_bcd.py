...
def dataloader(num_batches,
    ...
    for batch_num in range(num_batches):
        dice = torch.empty(1).uniform_(0, 7).item()
        if dice <= 1:
            # increase
            # All batches have the same sequence length and number of reps
            seq_len = random.randint(seq_min_len, seq_max_len)
            ...
            inp = np.zeros((seq_len + 1, batch_size, seq_width + 1))
            outp = np.zeros((seq_len + 2, batch_size, seq_width + 1))
            inp_numbers = []
            outp_numbers = []
            ...
            for i in range(batch_size):
                ...
                inp_number = str(inp_number).zfill(seq_len)
                outp_number = str(outp_number).zfill(seq_len + 1)
                inp_numbers.append(inp_number)
                outp_numbers.append(outp_number)
            ...
            outp[seq_len + 1, :, seq_width] = 1.0

            inp = torch.from_numpy(inp)
            outp = torch.from_numpy(outp)
            ctrl = np.array([[bcd_excess_3[3]]])
            ctrl = torch.from_numpy(ctrl)
...
@attrs
class AllBCDTaskParams(object):
    name = attrib(default="all-bcd")
...
@attrs
class AllBCDModelTraining(object):
    params = attrib(default=Factory(AllBCDTaskParams))
    net = attrib()
    dataloader = attrib()
    criterion = attrib()
    optimizer = attrib()
