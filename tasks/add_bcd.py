...
def dataloader(num_batches,
    ...
    for batch_num in range(num_batches):
        ...
        # All batches have the same sequence length and number of reps
        seq_len_1 = random.randint(seq_min_len, seq_max_len)
        seq_len_2 = random.randint(seq_min_len, seq_max_len)
        ...
        inp = np.zeros((seq_len + 4, batch_size, seq_width + 1))
        outp = np.zeros((max(seq_len_1, seq_len_2) + 2 + 0 , batch_size, seq_width + 1))
        inp_numbers_1 = []
        inp_numbers_2 = []
        outp_numbers = []

        for i in range(batch_size):
            inp_number_1 = np.random.randint(10 ** seq_len_1)
            inp_number_2 = np.random.randint(10 ** seq_len_2)
            outp_number = inp_number_1 + inp_number_2
            inp_number_1 = str(inp_number_1).zfill(seq_len_1)
            inp_number_2 = str(inp_number_2).zfill(seq_len_2)
            outp_number = str(outp_number).zfill(max(seq_len_1, seq_len_2))
            inp_numbers_1.append(inp_number_1)
            inp_numbers_2.append(inp_number_2)
            outp_numbers.append(outp_number)
        ...
        for i in range(seq_len_1):
            for j in range(batch_size):
                inp_digit = bcd_excess_3[int(inp_numbers_1[j][i])]
                inp[i+1, j, :seq_width] = inp_digit
        inp[seq_len_1 + 1, :, :] = 1.0

        inp[seq_len_1 + 2, :, seq_width] = 1.0
        for i in range(seq_len_2):
            for j in range(batch_size):
                inp_digit = bcd_excess_3[int(inp_numbers_2[j][i])]
                inp[i+seq_len_1 + 3, j, :seq_width] = inp_digit
        inp[seq_len_1 + seq_len_2 + 3, :, :] = 1.0

        # outp[0:10, :, :] = 0.0
        outp[0, :, seq_width] = 1.0
        for i in range(max(seq_len_1, seq_len_2)):
            for j in range(batch_size):
                outp_digit = bcd_excess_3[int(outp_numbers[j][i])]
                outp[0+i+1, j, :seq_width] = outp_digit
        outp[0+max(seq_len_1, seq_len_2)+1, :, :] = 1.0

        # plt.imshow(1 - inp[:,0,:], cmap='hot', interpolation='nearest')
        # plt.savefig('inp.png')
        # plt.imshow(1 - outp[:,0,:], cmap='hot', interpolation='nearest')
...
@attrs
class AddBCDTaskParams(object):
    name = attrib(default="add-bcd")
...
@attrs
class AddBCDModelTraining(object):
    params = attrib(default=Factory(AddBCDTaskParams))
    net = attrib()
    dataloader = attrib()
    criterion = attrib()
    optimizer = attrib()
