...
@attrs
class ReverseTaskParams(object):
    name = attrib(default="reverse-task")
...
@attrs
class ReverseTaskModelTraining(object):
    params = attrib(default=Factory(ReverseTaskParams))
    net = attrib()
    dataloader = attrib()
    criterion = attrib()
    optimizer = attrib()
    ...
    def input_human(self, net, number_str):
        ...
        for i in range(seq_len):
            ...
            # print(digit_bcd)
            # input()
            if digit_bcd in bcd_excess_3_str:
                outp_human = outp_human + bcd_excess_3_str[digit_bcd]
            else:
                outp_human = outp_human + '#'

        return outp_human


    def input_human_lstm(self, net, number_str):
        seq_width = 4
        seq_len = len(number_str)
        inp = np.zeros((seq_len + 1, 1, seq_width + 1))
        outp = np.zeros((seq_len + 1, 1, seq_width + 1))
